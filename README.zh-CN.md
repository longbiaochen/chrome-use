...
`chrome-use` 现在正式发布了 `chrome-inspect`：一套更快、更完整的真实页面 inspect 工作流，用来让 agent 和操作者协同处理线上或本地页面。
...
这个仓库是刻意有观点的：它不是一个泛化的浏览器 MCP 包装层，而是一套本地优先、强调更快 inspect 交接和稳定登录态的人机协作浏览器技能集。
