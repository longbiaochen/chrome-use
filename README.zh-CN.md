...
> `chrome-use` 是一套给 coding agent 用的托管 Chrome for Testing 工作流。
> 它通过一个持续存在的专用 agent 浏览器，把“人点页面”和“agent 接着做事”这两段流程接成一个稳定闭环。
...
- agent 可以在同一个托管浏览器里完成登录，不用每次重新起一个临时浏览器
...
在实现上，`chrome-inspect` 会把面板常驻在托管的 Chrome for Testing 会话里，持久化最近一次有效选择，并返回 `selectedElement`、页面信息、片段和位置等后续 DOM 修改真正需要的上下文。
...
_演示：打开本地 auth fixture，定位真正的 `Sign up` 入口，帮 `John Appleseed` 完成注册，进入登录页后再用同一账号登录，在托管的 Chrome for Testing 会话里把 auth 闭环跑完。_

在实现上，`chrome-auth` 运行在同一个托管 Chrome for Testing session 中，支持枚举和切换标签页、读取结构化快照、等待状态变化，并直接通过 CDP 执行 click / fill / type，而不是回退到靠截图做页面理解。

## General

## 🧠 架构

chrome-use 使用托管的 `Chrome for Testing` 浏览器，再加上一层受控的 CDP 运行时：

- 托管浏览器目录：`~/.chrome-use/browsers/chrome-for-testing/<version>/<platform>`
- 独立 browser-data 目录：`~/.chrome-use/browser-data/<channel-or-version>`
- state 目录：`~/.chrome-use/state`
- debug endpoint：`http://127.0.0.1:9223`
- 基于 CDP 的 remote debugging 会话

这样设计的意义在于：

- agent 始终工作在隔离的专用浏览器世界里
- 鉴权状态可以稳定复用，跨回合保留
...
在 macOS 上，启动器会直接拉起托管的 `Chrome for Testing` 二进制，并带上独立的 `--user-data-dir`。当 CDP 已经可用时，它会复用同一个 runtime。运行时仍然要求只有一个 owner process 对外暴露这个 debug endpoint，但当不同 thread 分别绑定到不同 target 时，同一个浏览器里可以保留多个窗口。

需要写清的边界：

- 这里的 `Chrome for Testing` 是自动化浏览器，不是你平时日常使用的 Chrome
- 它不会继承你现有 Chrome `Default` 里的 cookie、扩展、书签、密码或同步状态
- 它不会自动更新；需要时重新运行 `bash install/install.sh` 更新 Stable，或者通过 `CHROME_USE_CFT_VERSION` 固定版本
- 它更适合访问可信内容和 agent 工作流，不应该被宣传成通用日常浏览器
- 如果你明确要走用户自备浏览器路径，用 `--browser system` 或设置 `CHROME_USE_BROWSER_KIND=system`
...
| --- | --- | --- |
| Chrome DevTools MCP | 通用浏览器调试、自动化、trace、network、console、screenshot | chrome-use 增加了 inspect-first 的人机协作工作流、持久页面面板，以及可直接交给修改工具的 selection handoff，并且落在隔离的托管 agent 浏览器上 |
...
安装器会先把托管运行时发布到 `~/.chrome-use`，默认从官方 Chrome for Testing 源下载受管浏览器，再把公共 skills 物化复制到你选定的 agent 目录，并把结果写入 `~/.chrome-use/install-manifest.json`。
...
`chrome-auth` 则通过同一个已附着 Google Chrome 会话上的直接 CDP auth 辅助命令工作：
...
- browser kind：`cft`
- browser 目录：`~/.chrome-use/browsers/chrome-for-testing/<version>/<platform>`
- profile 目录：`~/.chrome-use/browser-data/<channel-or-version>`
- profile 名称：`Default`
- state 目录：`~/.chrome-use/state`
...
- 只能有一个 `Google Chrome for Testing` owner process 暴露 `127.0.0.1:9223`
- 该进程预期承载配置好的独立 browser-data 路径
- 所有公开的 inspect/auth/runtime 入口在附着前都会先做托管浏览器 preflight；如果当前 runtime 没有以可附着方式运行，运行时会先自动修复，再决定是否继续
- 后续启动必须复用同一个 owner process，并在该实例中打开新标签页
- 允许多个窗口，但工作流必须继续依赖 `targetId` / `bindingId` 绑定目标页；不再支持把“单窗口”当作契约本身
- 如果 canonical endpoint 在自动修复后仍不属于预期的 Chrome for Testing 进程，运行时会直接阻断，而不是静默降级到别的 profile 或错误端口
...
在 macOS 上，如果你想手动把托管浏览器引导到可附着状态，请直接运行：

```bash
~/.chrome-use/bin/chrome-use-open-google-chrome
```

这个入口会把托管的 `Chrome for Testing` 启动或重启到预期的 CDP 状态，并复用与 `chrome-inspect`、`chrome-auth` 相同的运行时。

如果你的 Codex 环境需要显式改用其他浏览器：

- 设置 `CHROME_USE_BROWSER_KIND=system` 使用自备浏览器，或
- 在本地 shell / profile 里预先导出 `CHROME_USE_CHROME_BIN` / `CHROME_USE_CHROME_APP`
...
2. 让 `scripts/open_url.sh` 打开托管浏览器；当配置了 `CHROME_INSPECT_PROJECT_ROOT` 或能从当前仓库推断出本地项目时，它会优先自动启动本地 web app。
   如果托管的 Chrome for Testing runtime 已经在运行，命令会在同一实例中复用并打开新标签页，而不是创建第二个专用窗口。
...
4. 确认 inspect mode 已经 armed，然后在 Chrome 中点击目标元素。
   ...
   页面应立即恢复可交互；面板也应跨同标签导航、刷新、同文档导航以及同一托管浏览器会话内其他标签持续存在。
...
- 同一托管 Chrome for Testing 会话中的每个页面标签都应默认收到同一个 idle 状态的面板。
...
对于 `/chrome-auth`，当已知目标 URL 时直接附上 URL 命令，然后通过同一个托管浏览器 session 里的 `scripts/auth-cdp` 完成状态检查、页面切换、结构化快照、截图、元素查找、点击、填充、输入、等待和按键操作。
...
如需验证 Chrome attach 运行时契约（使用 mocked process、endpoint 和 window state），请运行：
